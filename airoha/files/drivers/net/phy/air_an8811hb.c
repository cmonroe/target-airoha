...
static int __air_pbus_reg_modify(struct phy_device *phydev,
				 u32 pbus_address, u32 mask, u32 set)
{
	...
	/* Read current value */
	ret = __air_pbus_reg_read(phydev, pbus_address, &pbus_data_old);
	if (ret < 0)
		return ret;

	phydev_info(phydev, "%s %d: 0x%x 0x%x\n",
		    __func__, __LINE__, pbus_address, pbus_data_old);
	/* Calculate new value */
	...
	phydev_info(phydev, "%s %d: 0x%x 0x%x\n",
		    __func__, __LINE__, pbus_address, pbus_data_new);
	/* Write new value */
	return __air_pbus_reg_write(phydev, pbus_address, pbus_data_new);
}

int air_pbus_reg_modify(struct phy_device *phydev,
			unsigned int pbus_reg, unsigned int mask, unsigned int set)
{
	int ret = 0;
	struct device *dev = phydev_dev(phydev);
	struct mii_bus *mbus = phydev_mdio_bus(phydev);
	...
	mutex_lock(&mbus->mdio_lock);
	ret = __air_pbus_reg_modify(phydev, pbus_reg, mask, set);
	mutex_unlock(&mbus->mdio_lock);
	if (ret) {
		dev_err(dev, "__air_pbus_reg_modify, ret: %d\n", ret);
		return ret;
	}
	return ret;
}
...
int an8811hb_set_zero(struct phy_device *phydev)
{
	struct mii_bus *mbus = phydev_mdio_bus(phydev);
	int ret;
...
int an8811hb_set_one(struct phy_device *phydev)
{
	struct mii_bus *mbus = phydev_mdio_bus(phydev);
	int ret;
