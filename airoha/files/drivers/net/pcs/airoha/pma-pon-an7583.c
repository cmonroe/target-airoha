...
#include <linux/pcs/pma-pon-an7583.h>
#include <linux/pcs/ecnt_event_serdes_sel.h>
...
static uint en7583_bist_check_err_cnt;  
...
*************************************************************************
*/
extern void __iomem* Get_PMA_Base(u32 base);
extern u32 get_phy_efuse(u32 start_bit, u32 len);

extern void SET_FORCE_GPIO32_EN(u32 mask, u32 bits);

extern uint8_t* get_board_config_variable(void);
...
{
	pon_ana = Get_PMA_Base(PON_ANA_RG_BASE); 
	if (IS_ERR(pon_ana))
	{
...
{
	pon_pma = Get_PMA_Base(PON_PMA_RG_BASE); 
	if (IS_ERR(pon_pma))
	{
...
{
   uint8_t* pboard_config = NULL;
   
   pboard_config = get_board_config_variable();
...
/*
	...
	if( Rfb_id == 2)
	{
	     ...
	     {
*/
	if(pboard_config && pboard_config[0] == 0x1)
	{
...
/* TODO: Disable Efuse for AN7583 bring up */
...
//
void pon_xfi_Ext_T2R (int pon_Spd)
{  
...
//================= ASIC_PON_SERDES_API =================
/* TODO: Discard this API to aviod compilation error while AN7583 PMA bring up */
#if 1
void normal_rx_bist_check(uint BIST_check_switch, XPON_PMA_RX_Bist_Check_t align_pattern)
{
	...
	//wan_sel = GET_WAN_CONF()&SCU_WAN_CONF_REG_WAN_SEL_BITS;
	wan_sel = SCU_WAN_CONF_REG_WAN_SEL_XGSPON;
...
int xpon_bist_check_pattern_sel(XPON_PMA_RX_Bist_Check_t align_pattern)
{
	...
	switch(align_pattern)
	{
		case PRBS7_ALIGN_PATTERN:
			...
			IO_SPHYA_REG_BITS(AN7583_PON_PMA_BISTCTL_ALIGN_PAT,	 31,	0,	0x6081FD53);//rg_px_lnx_bist_ctl_align_pattern	
			break;
	...
	IO_SPHYA_REG_BITS(AN7583_PON_PMA_BISTCTL_PRBS_INITIAL_SEED, 	31, 0,	0x8FF1FD53);//bistctl_prbs_init_seed	
...
void xpon_rx_bist_recheck_result(uint check_onoff, uint duration)
{
	...
	if(check_onoff == PHY_ENABLE)
	{
		...
		en7583_bist_check_err_cnt = IO_GPHYA_REG_BITS(AN7583_PON_PMA_ADD_DIG_RO_RESERVE_1,	31, 0);

		//phy_print_time();
		printk("PRBS Error Counter = %u \n",en7583_bist_check_err_cnt); //unsigned output
...
{
	uint prbs_result = 0 , read_data = 0 , i =0;
	...
	if(BIST_check_switch == PHY_ENABLE)
	{
		...
		IO_SPHYA_REG_BITS(AN7583_PON_PMA_BIST_1, 31, 0, 0x0); 	         // BFAF3684 default = 0x01000000
...
/* TODO: Discard this API to aviod compilation error while AN7583 PMA bring up */
#if 0
void TX_Dis_off (uint tx_dis_inv )
{
     SET_FORCE_GPIO32_EN(EN7583_XPON_SCU_GPIO_MASK,tx_dis_inv);
}
EXPORT_SYMBOL(TX_Dis_off);
#endif


void TX_Ben_Ctrl (void)
{

	IO_SPHYA_REG_BITS(AN7583_PON_PMA_XPON_SETTING_0, 8, 8, 0x00); //rg_trans_burst_inv	
	IO_SPHYA_REG_BITS(AN7583_PON_PMA_BEN_CTRL_0, 0, 0, 0x01); //rg_da_ben_force_mode
}
EXPORT_SYMBOL(TX_Ben_Ctrl);





void Pon_RX_Data_INV (uint data_inv )
{

    IO_SPHYA_REG_BITS(AN7583_PON_PMA_BISTCTL_CONTROL, 25,24 , data_inv );				 // rg_bistctl_io_data_inv
    IO_SPHYA_REG_BITS(AN7583_PON_PMA_ADD_DIG_RESERVE_0, 0,0 , data_inv );				 // rg_RX_data_inv
}
EXPORT_SYMBOL(Pon_RX_Data_INV);
